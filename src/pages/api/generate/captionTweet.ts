...
import {
  getExampleMessagesGenerateCaptionTweet,
  getSystemMessageGenerate,
  getSystemMessageRulesCaptionTweet,
} from '@/data/prompts'
...
export default async function handler(req: ReqData, res: NextApiResponse<ResData>) {
  ...
  try {
    ...
    const SYSTEM_MESSAGE_GENERATE_CAPTION_RULES = getSystemMessageRulesCaptionTweet(config.actor)
    const EXAMPLE_MESSAGES_GENERATE_CAPTION = getExampleMessagesGenerateCaptionTweet(config.actor)
